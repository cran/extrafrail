...
summary.extrafrail=function (object, ...) 
{
        ...
        if (uni) 
            cat("Univariate weighted Lindley frailty model with\n", 
                ifelse(object$dist == "np", "non-parametric", 
                  ifelse(object$dist=="pe","piecewise exponential",object$dist)), " survival function\n", sep = "")
        if (bi) 
    ...
    if (object$dist.frail == "GA") {
        if (uni) 
            cat("Univariate gamma frailty model with\n", ifelse(object$dist == 
                "np", "non-parametric", ifelse(object$dist=="pe","piecewise exponential",object$dist)), " survival function\n", 
                sep = "")
        if (bi) 
	...
	if (object$dist == "pe") {
            if (nrow(tt) > length(object$part)+1) {
                cat("-------------------------------------------------------------------------\n")
                cat("Regression Coefficients\n")
                print(tt[-c(nrow(tt) - 0:(length(object$part))), , drop = FALSE])
                cat("---\n")
                cat("Signif. codes:  0 \"***\" 0.001 \"**\" 0.01 \"*\" 0.05 \".\" 0.1 \" \" 1\n")
            }
	        }
