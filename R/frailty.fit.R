    ...
    max.iter = 1000, part=NULL) 
{
    ...
    H.base <- function(t, lambda, rho=1, dist, part=NULL) {
        ...
        if (dist == "pe") {Lambda0 <- -ppexp(t, rate=lambda, t=part, lower.tail=FALSE, log.p=TRUE)}
        Lambda0
    }
    h.base <- function(t, lambda, rho=1, dist, log = TRUE, part=NULL) {
        if (dist == "weibull") {log.lambda0 <- log(lambda) + log(rho) + (rho - 1) * log(t)}
        if (dist == "pe") {log.lambda0 <- dpexp(t, rate=lambda, t=part, log=TRUE)-ppexp(t, rate=lambda, t=part, lower.tail=FALSE, log.p=TRUE)}
        log.lambda0
    }
    frailtyGA <- function(formula, data, dist = "np", prec = 1e-04, 
		...
		if(dist=="exponential"){dist="pe"; part=0; dist.aux="exponential"}
			...
			theta.last), t = t, delta = delta, 
                ...
                se = sqrt(diag(solve(aux.se)))
                llike.obs = -observed.llike.0.ga.dist(c(lambda.last, theta.last), t = t, delta = delta, 
                  ind = cluster, dist = dist, part=part)
